...
export default function Compare({ before, after, alt }: { before: string; after: string; alt: string }) {
  ...
  return (
    <div
      ref={ref}
      className="relative aspect-[1600/953] w-full cursor-col-resize select-none overflow-hidden rounded-[20px] border border-line bg-paper-deep"
    ...
    >
      <Image src={before} alt="" fill sizes="(min-width: 1024px) 760px, 100vw" className="object-cover" />
      ...
      <button
        ...
        onKeyDown={(e) => { if (e.key === "ArrowLeft") setPos((p) => Math.max(3, p - 3)); if (e.key === "ArrowRight") setPos((p) => Math.min(97, p + 3)); }}
